...
Powered by GroqCloud (Free Tier) using GPT-OSS-120B.
...
if not TELEGRAM_TOKEN or not GROQ_API_KEY:
    raise ValueError("TELEGRAM_TOKEN and GROQ_API_KEY must be set.")
...
# ========== RATE LIMITER (only for new draft requests) ==========
new_request_times = defaultdict(list)

def check_new_request_limit(user_id: int, max_per_minute: int = 10, max_per_day: int = 20) -> tuple[bool, str]:
    now = datetime.now()
    new_request_times[user_id] = [
        t for t in new_request_times[user_id] if now - t < timedelta(days=1)
    ]
    times = new_request_times[user_id]
    
    minute_ago = now - timedelta(minutes=1)
    recent = [t for t in times if t > minute_ago]
    if len(recent) >= max_per_minute:
        wait_seconds = 60 - (now - min(recent)).seconds
        return False, f"⚠️ बहुत तेज! कृपया {wait_seconds} सेकंड रुकें।"
    
    if len(times) >= max_per_day:
        return False, "⚠️ आज की 20 ड्राफ्ट की सीमा पूरी हो गई। कल पुनः प्रयास करें।"
...
अब उपरोक्त सभी निर्देशों का पालन करते हुए एक संपूर्ण और प्रोफेशनल दस्तावेज़ तैयार करें। **दस्तावेज़ के बाद** निम्नलिखित तीन चीज़ें भी बताएँ:

**LEGAL_BASIS:** (इस्तेमाल किए गए कानूनी प्रावधान, यदि कोई)
**ASSUMPTIONS:** (जो धारणाएँ ली गई हैं)
**MISSING_INFORMATION:** (कौन-सी जानकारी गायब थी)
"""
...
1. **सेवा का स्वरूप:** यह बॉट एक AI-संचालित दस्तावेज़ प्रारूप सहायक (AI Legal Draft Assistant) है। यह केवल दस्तावेज़ों का मसौदा तैयार करता है और किसी भी प्रकार की कानूनी सलाह, वकालत या विधिक सेवा प्रदान नहीं करता है।
...
5. **AI प्रसंस्करण:** आपके टेक्स्ट को ड्राफ्ट तैयार करने के लिए GroqCloud AI API को भेजा जाता है।
"""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'fields' in context.user_data:
        # बीच में फील्ड भरते समय कोई रेट लिमिट नहीं
        await collect_next_field(update, context)
    else:
        allowed, msg = check_new_request_limit(user_id)
        if not allowed:
...
async def generate_final_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if meta_parts:
            await update.message.reply_text("\n\n".join(meta_parts), parse_mode='Markdown')
