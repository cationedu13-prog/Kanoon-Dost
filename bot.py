...
TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKEN")   # Render में ज़रूर सेट करें
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY")   # Render में ज़रूर सेट करें
ADMIN_CHAT_ID = os.environ.get("ADMIN_CHAT_ID")     # Optional: आपकी चैट ID (पेमेंट नोटिफिकेशन के लिए)

# अगर टोकन नहीं मिला तो बॉट चल नहीं पाएगा (सुरक्षा के लिए)
if not TELEGRAM_TOKEN or not GEMINI_API_KEY:
    raise ValueError("TELEGRAM_TOKEN and GEMINI_API_KEY must be set in environment variables.")

# Gemini Setup
genai.configure(api_key=GEMINI_API_KEY)
model = genai.GenerativeModel("gemini-1.5-flash")  # या "gemini-2.5-flash" (free tier)
...
आप एक अनुभवी भारतीय कानूनी ड्राफ्टिंग सहायक हैं। इस अनुरोध के आधार पर एक पेशेवर दस्तावेज़ तैयार करने के लिए वास्तव में किन-किन जानकारियों (fields) की आवश्यकता होगी?
कृपया केवल और केवल एक JSON ऐरे के रूप में उत्तर दें, जिसमें हर फील्ड का एक संक्षिप्त विवरण और एक उदाहरण शामिल हो।
उदाहरण स्वरूप:
[
    "प्रेषक का पूरा नाम (उदाहरण: रोहित शर्मा)",
    "प्रेषक का पूरा पता (उदाहरण: 12, मेन रोड, मुंबई)",
    ...
]
- यदि दस्तावेज़ कानूनी प्रकृति का है, तो आवश्यक विवरण (जैसे, विवाद की राशि, तिथियाँ, समझौते का विवरण) ज़रूर शामिल करें।
- केवल JSON ऐरे लौटाएँ, कोई अतिरिक्त टेक्स्ट, स्पष्टीकरण या मार्कडाउन नहीं।
- फील्ड का नाम हिंदी या अंग्रेजी में रखें, जो भी उपयोगकर्ता ने अपने संदेश में उपयोग किया हो।
"""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """नए यूज़र के लिए स्वागत संदेश और निर्देश।"""
    # हर बार नए सेशन पर पुराना डेटा साफ करें
...
async def start_new_request(update: Update, context: ContextTypes.DEFAULT_TYPE, user_text: str):
    """नए दस्तावेज़ के लिए Gemini से ज़रूरी फील्ड्स की सूची लें।"""
    ...
    try:
        ...
        # अगर Gemini ने मार्कडाउन कोड ब्लॉक में दिया है तो साफ करें
        if raw.startswith("```"):
            raw = raw.split("\n", 1)[1].rsplit("```", 1)[0].strip()
        fields = json.loads(raw)
        if not isinstance(fields, list) or len(fields) == 0:
            raise ValueError("Invalid fields list")
    except Exception as e:
        logger.error(f"Field extraction error: {e}, raw response: {raw if 'raw' in locals() else 'none'}")
        await update.message.reply_text(
            "❌ क्षमा करें, आपके अनुरोध को समझने में दिक्कत हुई। कृपया थोड़ा और स्पष्ट लिखें या /start करके पुनः प्रयास करें।"
        )
        return

    # सब कुछ स्टोर करें और पहला सवाल पूछें
    context.user_data['fields'] = fields
    context.user_data['collected_data'] = {}
    context.user_data['current_index'] = 0
    context.user_data['original_request'] = user_text

    # पुराने डिलीट जॉब (यदि कोई हों) हटाएँ
...
async def collect_next_field(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """यूज़र का जवाब स्टोर करें और अगला फील्ड पूछें या फाइनल ड्राफ्ट बनाएँ।"""
    ...
    # वर्तमान फील्ड का जवाब सेव करें
    ...
    if next_idx < len(fields):
        ...
        # सब जवाब मिल गए, फाइनल ड्राफ्ट बनाएँ
...
async def generate_final_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """सारी जानकारी को Gemini को भेजकर अंतिम दस्तावेज़ तैयार करें।"""
    user_data = context.user_data
    await update.message.reply_text("⏳ सारी जानकारी मिल गई। अब आपका डॉक्यूमेंट तैयार कर रहा हूँ...")

    # सारी फील्ड्स और जवाबों से details_text बनाएँ
    details_lines = []
    ...
    try:
        ...
        # 1 घंटे बाद डेटा डिलीट करने का जॉब सेट करें
...
def schedule_data_deletion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """1 घंटे बाद यूज़र का डेटा अपने आप मिटाने का जॉब सेट करें।"""
    user_id = update.effective_user.id

    # पहले से मौजूद जॉब हटाएँ
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # एडमिन को मैसेज (अगर ADMIN_CHAT_ID सेट है)
    if ADMIN_CHAT_ID:
        try:
            admin_msg = (
            ...
            # फोटो भी फॉरवर्ड करें (वैकल्पिक)
            await update.message.forward(ADMIN_CHAT_ID)
        except Exception as e:
            logger.error(f"Failed to notify admin: {e}")

# ========== MAIN APPLICATION (WEBHOOK SETUP) ==========
def main():
    """बॉट शुरू करें - Webhook मोड में।"""
    # Application बनाएं
    ...
    # टेक्स्ट मैसेज हैंडलर (फील्ड कलेक्शन और नए अनुरोध)
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message))

    # फोटो हैंडलर (पेमेंट स्क्रीनशॉट)
    application.add_handler(MessageHandler(filters.PHOTO, handle_photo))

    # ---------- WEBHOOK CONFIGURATION ----------
    # Render जो PORT प्रदान करता है (डिफ़ॉल्ट 10000, हम 8080 भी रख सकते हैं)
    PORT = int(os.environ.get("PORT", 8080))
    # Render का एक्सटर्नल URL (यह Render खुद सेट करता है, हमें बस उपयोग करना है)
    RENDER_URL = os.environ.get("RENDER_EXTERNAL_URL")
    if not RENDER_URL:
        # लोकल टेस्टिंग के लिए fallback (localhost), लेकिन Render पर यह सेट रहेगा
